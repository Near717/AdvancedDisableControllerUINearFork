...
local originalActionButton_SetupSwapAnimation = ActionButton["SetupSwapAnimation"]
local function myActionButton_SetupSwapAnimation(self, OnStopHandlerFirst, OnStopHandlerLast)
  if not ADCUI:originalIsInGamepadPreferredMode() then
    return originalActionButton_SetupSwapAnimation(self, OnStopHandlerFirst, OnStopHandlerLast)
  end
...
-- for some reason once we set the swap animations we have to keep updating every gamepad mode change
-- if registered once then never unregister
local function onGamepadModeChanged_ActionBarSwapAnimation()
...
function ADCUI:setGamepadActionBarOverrideState(state)
  if state then
    ...
    ActionButton["SetupSwapAnimation"] = myActionButton_SetupSwapAnimation
    ...
    if not registeredOnce then
      EVENT_MANAGER:RegisterForEvent(ADCUI.const.ADDON_NAME .. "_ActionBarSwapAnimation", EVENT_GAMEPAD_PREFERRED_MODE_CHANGED, onGamepadModeChanged_ActionBarSwapAnimation)
      registeredOnce = true
    end
  else
    ...
    ActionButton["SetupSwapAnimation"] = originalActionButton_SetupSwapAnimation
